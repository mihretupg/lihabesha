...
def create_housing(payload: HousingPostCreate, db: Session = Depends(get_db)):
...
def update_housing(post_id, payload: PostUpdate, db: Session = Depends(get_db)):
...
def delete_housing(post_id, db: Session = Depends(get_db)):
    post = post_service.get_post(db, post_id)
    if not post:
        raise HTTPException(status_code=404, detail="Post not found")
